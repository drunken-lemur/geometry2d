...
describe("Point", () => {
  ...
  it("static plus", () => {
    expect(Point.get(1)).toMatchObject({ x: 1, y: 1 });
    expect(Point.get(2, 3)).toMatchObject({ x: 2, y: 3 });
    expect(Point.get({ x: 7, y: 9 })).toMatchObject({ x: 7, y: 9 });
    expect(Point.get({ x: 2, y: 1 })).toMatchObject({ x: 2, y: 1 });
    expect(Point.get(() => ({ x: 2, y: 1 }))).toMatchObject({ x: 2, y: 1 });
  });
  ...
  it("static plus", () => {
    expect(Point.plus({ x: 0, y: 0 }, { x: 1, y: 1 })).toMatchObject({ x: 1, y: 1 });
    expect(Point.plus({ x: 2, y: 1 }, { x: 1, y: 2 })).toMatchObject({ x: 3, y: 3 });
    expect(Point.plus(() => ({ x: 2, y: 1 }), () => ({ x: 1, y: 2 }))).toMatchObject({ x: 3, y: 3 });
  });

  it("static minus", () => {
    expect(Point.minus({ x: 0, y: 0 }, { x: 1, y: 1 })).toMatchObject({ x: -1, y: -1 });
    expect(Point.minus({ x: 2, y: 1 }, { x: 1, y: 2 })).toMatchObject({ x: 1, y: -1 });
    expect(Point.minus(() => ({ x: 2, y: 1 }), () => ({ x: 1, y: 2 }))).toMatchObject({ x: 1, y: -1 });
  });

  it("static multiply", () => {
    expect(Point.multiply({ x: 1, y: 2 }, { x: 3, y: 4 })).toMatchObject({ x: 3, y: 8 });
    expect(Point.multiply({ x: 4, y: 3 }, { x: 1, y: 2 })).toMatchObject({ x: 4, y: 6 });
    expect(Point.multiply(() => ({ x: 4, y: 3 }), () => ({ x: 1, y: 2 }))).toMatchObject({ x: 4, y: 6 });
  });

  it("static divide", () => {
    expect(Point.divide({ x: 3, y: 8 }, { x: 3, y: 4 })).toMatchObject({ x: 1, y: 2 });
    expect(Point.divide({ x: 4, y: 6 }, { x: 1, y: 2 })).toMatchObject({ x: 4, y: 3 });
    expect(Point.divide(() => ({ x: 4, y: 6 }), () => ({ x: 1, y: 2 }))).toMatchObject({ x: 4, y: 3 });
  });

  it("static min", () => {
    expect(Point.min({ x: 2, y: 4 }, { x: 5, y: 3 })).toMatchObject({ x: 2, y: 3 });
    expect(Point.min({ x: 5, y: 4 }, { x: 2, y: 3 })).toMatchObject({ x: 2, y: 3 });
    expect(Point.min(() => ({ x: 5, y: 4 }), () => ({ x: 2, y: 3 }))).toMatchObject({ x: 2, y: 3 });
  });

  it("static max", () => {
    expect(Point.max({ x: 2, y: 4 }, { x: 5, y: 3 })).toMatchObject({ x: 5, y: 4 });
    expect(Point.max({ x: 2, y: 4 }, { x: 5, y: 3 })).toMatchObject({ x: 5, y: 4 });
    expect(Point.max(() => ({ x: 2, y: 4 }), () => ({ x: 5, y: 3 }))).toMatchObject({ x: 5, y: 4 });
  });
  ...
  it("static values", () => {
    ...
    expect(Point.values(new Point(1, 2))).toMatchObject([1, 2]);
    expect(Point.values(() => ({ x: 1, y: 2 }))).toMatchObject([1, 2]);
  });
  ...
  it("static convert", () => {
    expect(Point.convert(({ x, y }) => ({ a: y, b: x }), { x: 13, y: 73 })).toMatchObject({ a: 73, b: 13 });
  ...
  it("static ofAngle", () => {
    expect(Point.round(Point.ofAngle(45, 42, true))).toMatchObject({ x: 30, y: 30 });
    expect(Point.round(Point.ofAngle(90, 80, true))).toMatchObject({ x: 0, y: 80 });
  });
  ...
  it("set", () => {
    expect(new Point().set(1, 1)).toMatchObject({ x: 1, y: 1 });
  ...
  it("minus", () => {
    ...
    expect(new Point(2, 1).minus({ x: 1, y: 2 })).toMatchObject({ x: 1, y: -1 });
    expect(new Point(2, 1).minus(() => ({ x: 1, y: 2 }))).toMatchObject({ x: 1, y: -1 });
  });

  it("multiply", () => {
    expect(new Point(1, 2).multiply({ x: 3, y: 4 })).toMatchObject({ x: 3, y: 8 });
    expect(new Point(4, 3).multiply({ x: 1, y: 2 })).toMatchObject({ x: 4, y: 6 });
    expect(new Point(4, 3).multiply(() => ({ x: 1, y: 2 }))).toMatchObject({ x: 4, y: 6 });
  });

  it("divide", () => {
    expect(new Point(3, 8).divide({ x: 3, y: 4 })).toMatchObject({ x: 1, y: 2 });
    expect(new Point(4, 6).divide({ x: 1, y: 2 })).toMatchObject({ x: 4, y: 3 });
    expect(new Point(4, 6).divide(() => ({ x: 1, y: 2 }))).toMatchObject({ x: 4, y: 3 });
  });
  ...
  it("random", () => {
    ...
    const d = new Point(15, 15);
    const b = a.clone();

    expect(a).toMatchObject(b);
    expect(a.random()).not.toMatchObject(b);
    expect(a.random(d, c)).not.toMatchObject(b);
    expect(a.random(() => ({ x: 15, y: 15 }))).not.toMatchObject(b);
  });
  ...
  it("isLess", () => {
    ...
    expect(new Point(10, 10).isLess(() => Point.get(11, 11))).toBeTruthy();
    expect(new Point(10, 10).isLess(new Point(10, 10))).toBeFalsy();
    expect(new Point(10, 10).isLess(() => Point.get(10, 10))).toBeFalsy();
    expect(new Point(10, 10).isLess(new Point(10, 10), true)).toBeTruthy();
    expect(new Point(10, 10).isLess(() => Point.get(10, 10), true)).toBeTruthy();
    expect(new Point(10, 10).isLess(new Point(10, 9), false, false)).toBeFalsy();
    expect(new Point(10, 10).isLess(() => Point.get(10, 9), false, false)).toBeFalsy();
    expect(new Point(10, 10).isLess(new Point(10, 9), true, false)).toBeTruthy();
    expect(new Point(10, 10).isLess(() => Point.get(10, 9), true, false)).toBeTruthy();
  });

  it("isMore", () => {
    expect(new Point(10, 10).isMore(new Point(9, 9))).toBeTruthy();
    expect(new Point(10, 10).isMore(() => Point.get(9, 9))).toBeTruthy();
    expect(new Point(10, 10).isMore(new Point(10, 10))).toBeFalsy();
    expect(new Point(10, 10).isMore(() => Point.get(10, 10))).toBeFalsy();
    expect(new Point(10, 10).isMore(new Point(10, 10), true)).toBeTruthy();
    expect(new Point(10, 10).isMore(() => Point.get(10, 10), true)).toBeTruthy();
    expect(new Point(10, 10).isMore(new Point(10, 11), false, false)).toBeFalsy();
    expect(new Point(10, 10).isMore(() => Point.get(10, 11), false, false)).toBeFalsy();
    expect(new Point(10, 10).isMore(new Point(10, 11), true, false)).toBeTruthy();
    expect(new Point(10, 10).isMore(() => Point.get(10, 11), true, false)).toBeTruthy();
  });
  ...
  it("moveToAngle", () => {
    ...
    expect(new Point().moveToAngle(1, 80).round()).toMatchObject({ x: -80, y: 0 });
  });
});
